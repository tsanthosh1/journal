...
import { syncSwiggyOrders } from "@/lib/swiggy/sync";
...
export async function POST(request: NextRequest) {
  if (!(await isAuthorizedUser(request))) {
    return unauthorizedResponse(
      "Authentication required to sync Swiggy orders"
    );
  }
  ...
  try {
    ...
    const result = await syncSwiggyOrders(userId, {
      maxResults,
      fullSync,
    });
    ...
    console.error("Error syncing Swiggy orders:", error);
