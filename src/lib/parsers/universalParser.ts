...
export class UniversalAutoParser implements IStatementParser {
  ...
  readonly description =
    "Automatically recognizes bank card e-statements, UPI transaction receipts, utility bills, jewellery savings schemes, apartment maintenance receipts, telecom and Airtel postpaid invoices.";
  ...
  parseStatement(content: string, subject = ""): ParsedStatement {
    ...
    const prioritizedParsers = [...this.statementParsers].sort((a, b) => {
      const aMatch =
        (a.id === "AirtelPostpaidParser" && textLower.includes("airtel")) ||
      ...
      const bMatch =
        (b.id === "AirtelPostpaidParser" && textLower.includes("airtel")) ||
